...
\institute[Inria and Cerema]{\textsuperscript{1}Inria, Université Côte d'Azur, \textsc{Ayana} research team, France
  \qquad\textsuperscript{2}Cerema, \textsc{Endsum} research team, France}

\hypersetup{pdfauthor={Louis Hauseux; Raphaël Antoine; Philippe Foucher; Pierre Charbonnier; Josiane Zerubia}}
\begin{document}
\begin{frame}[t]
% Balance the white space between the content and the two navy banners.
\vspace{0.7cm}
\begin{block}{FIND first example.}
...
\par{\captiontext Inputs and ground truth: FIND~\cite{zhou2022find}. Processing and extraction: our method.}
\end{block}

\begin{posterpanel}{\fusepanel}
\begin{block}{1. Fuse the modalities}
\begin{minipage}[t]{0.51\linewidth}\RaggedRight
\textbf{Normalize each modality.}
\[
...
\textbf{Fuse the Hessians.}
\[
\Hess_\sigma^{\mathrm{fused}}=\sum_{m\in\mathcal M}w_m\widehat\Hess_\sigma^{(m)}.
\]
\end{minipage}
\par{\smalltext Aligned dark-ridge inputs; $w_m\geq0$, $\sum_m w_m=1$.}
\par\vspace{0.15cm}
\begin{minipage}[c]{0.30\linewidth}\centering
  \tikzfigure{\linewidth}{frangi-hessian}
\end{minipage}\hfill
\begin{minipage}[c]{0.65\linewidth}\RaggedRight
  \textbf{Extract the geometry after fusion.}\par
  At scale $\sigma$: $|\lambda_1^\sigma|\leq|\lambda_2^\sigma|$.\par
  \textbf{Ridge direction:} $\theta^\sigma$.
\end{minipage}
\end{block}
\end{posterpanel}

\begin{posterpanel}{\linkpanel}
\begin{block}{2. From pixel-wise \textmd{\textsc{Frangi}} to pairwise links}
\centering\tikzfigure{0.86\linewidth}{frangi-terms}\par\RaggedRight
\par{\smalltext \textsc{Frangi}~\cite{frangi1998}: intensity and shape. \textbf{Our graph adds alignment.}
For $0<\rho_{ij}=\|x_i-x_j\|_2\leq R$:}
\[
...
\toprule
Method & Jaccard $\uparrow$ & Tversky $\uparrow$ & Wasserstein $\downarrow$\\
\midrule
% Ours: best configuration in the paper's modality-ablation table.
% CrackSegDiff: the paper's clean-data comparison table.
Ours, no training & 63\% & 71\% & 11 px\\
CrackSegDiff~\cite{jiang2024cracksegdiff} & \textbf{87\%} & \textbf{90\%} & \textbf{5 px}\\
\bottomrule
\end{tabular*}}
\par{\captiontext FIND: $256\!\times\!256$. Overlap: both skeletons dilated by 3 px;
Tversky: $\alpha=1$, $\beta=0.5$; Wasserstein: skeleton distance.}
\end{block}
\end{posterpanel}
...
\begin{block}{6. Geological domain shift}
\textbf{Our evaluation: Vaches Noires.} Two Cerema UAV patches ($512\!\times\!512$).
\par\vspace{0.2cm}
...
\par\smallskip U-Net adapted by us on a separate image; transfer inspired by Shi et al.~\cite{shi2023geocrack}.}
\end{minipage}
...
\begin{minipage}[c]{0.415\linewidth}\RaggedRight
  ...
  \textbf{Train LoRA~\cite{hu2022lora} and our coefficient $\beta$ together.}
  \[A_H=\mathrm{softmax}(L+\beta B_H).\]
  {\smalltext $L$: visual scores. \textbf{Our proposal:} $B_H$ from Frangi-graph merge heights.\par
  Ideas used: maximin connectivity (MALIS~\cite{turaga2009malis}) and attention biases
  (Graphormer~\cite{ying2021graphormer}).
  CrackSAM~\cite{ge2024cracksam}: SAM adaptation to cracks.\par
...
\textbf{Background review:} Zhang et al.~\cite{zhang2025review}.\par\smallskip
\begin{multicols}{2}
