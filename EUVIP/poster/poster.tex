...
\textbf{Fuse geometry, connect compatible pixels, extract a centerline.} No training labels required.
\par\vspace{0.25cm}
...
\begin{minipage}[c]{0.63\linewidth}\RaggedRight
  Hessian geometry [1,5] at scale $\sigma$:
...
\end{tikzpicture}}
\par\RaggedRight
Retain strong nodes and links. Per component: minimum spanning tree [6] using $d_{ij}$,
then similarity-weighted centrality to prune branches.
{\captiontext Schematic. FIND settings:
...
{\smalltext \textbf{Palais des Papes.} Main fracture recovered; minor cracks missed.}
\end{minipage}
\end{block}
\end{column}
...
\par\smallskip Transfer strategy [8]; local U-Net trained on a separate image.}
\end{minipage}
...
Transfer evidence: two patches and one qualitative wall example.}
\end{block}
\end{column}
\end{columns}
...
\begin{minipage}[c]{0.485\linewidth}
  \centering\tikzfigure{0.79\linewidth}{foundation-hierarchy}
\end{minipage}\hfill
\begin{minipage}[c]{0.49\linewidth}
  Could \textbf{nested Frangi groups} guide SAM [7] / CrackSAM [4] attention,
  from local segments to the whole network?
  \par\smallskip\textbf{Hypothesis: not evaluated here.}
\end{minipage}
\end{alertblock}
...
Constr. Build. Mater. 431, 136573, 2024.
\end{minipage}\hfill
\begin{minipage}[t]{0.485\linewidth}
[5] Sato et al., \emph{Three-dimensional multi-scale line filter for segmentation and visualization of
curvilinear structures in medical images}, Med. Image Anal. 2(2), 1998.\par
[6] Prim, \emph{Shortest connection networks and some generalizations}, Bell Syst. Tech. J. 36(6), 1957.\par
[7] Kirillov et al., \emph{Segment Anything}, ICCV, 2023.\par
[8] Shi et al., \emph{Semi-universal geo-crack detection by machine learning}, Front. Earth Sci. 11, 1073211, 2023.
\end{minipage}
\par\smallskip
