...
\begin{figure}[!t]
    ...
    \end{tikzpicture}
    \caption{Method overview. Colors follow Secs.~III-A--D (purple/blue/orange/green); Sec.~III-E summarizes their roles. Parallel black/gray arrows denote multiple modalities and retained components.}
...
For FIND and the Palais des Papes (\emph{cf.} Sec.~\ref{sec:datasets}), we use $s_{\mathrm{s}}=2$, $s_{\mathrm{i}}=1/4$, $s_{\mathrm{a}}=1/8$, $\Sigma=\{1,3,5,7\}$ and $R=3$. For the Vaches Noires images (\emph{cf.} Sec.~\ref{sec:datasets}), the single-modal implementation uses $s_{\mathrm{s}}=1/2$, $s_{\mathrm{i}}=1/4$, $s_{\mathrm{a}}=1/8$, $\Sigma=\{1,3,5,7,9\}$ to cover wider visible fractures, with the same radius $R=3$ as in the preliminary study.
