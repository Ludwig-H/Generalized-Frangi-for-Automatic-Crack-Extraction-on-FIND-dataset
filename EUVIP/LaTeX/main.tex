...
\author{
\IEEEauthorblockN{Louis Hauseux\IEEEauthorrefmark{1},
Raphaël Antoine\IEEEauthorrefmark{2},
Philippe Foucher\IEEEauthorrefmark{2},
Pierre Charbonnier\IEEEauthorrefmark{2}, and
Josiane Zerubia\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}\textit{Inria, Université Côte d'Azur, Ayana team, France}\\
\IEEEauthorrefmark{2}\textit{Cerema, ENDSUM research team, France}\\
Corresponding author: louis.hauseux@inria.fr}
}
...
While supervised models perform best on clean, in-distribution data, our geometry-driven framework degrades more slowly than CrackSegDiff under the tested synthetic perturbations. The geological experiments are limited in size and should be read as initial transfer tests rather than broad evidence of cross-domain generalization.
\end{abstract}
...
This limitation is especially relevant for multi-modal crack analysis, where public benchmarks with clean, well-aligned visible, depth or thermal modalities are still rare.
...
Multi-scale Hessian filters are a standard tool for enhancing curvilinear structures. The classical \Frangi{} vesselness filter \cite{Frangi1998} estimates whether a pixel belongs to a tubular structure from the eigenvalues of a smoothed Hessian matrix, following related work such as Sato's line filter \cite{Sato1998}. These methods are fast and interpretable, but they are local: they evaluate pixels independently and can fragment under noise or textured backgrounds. Our method keeps the Hessian geometry but moves the decision from isolated pixels to edges of a spatial graph.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[
        ...
        flow/.style={-{Latex[length=1.5mm]}, line width=0.35pt},
        node distance=2mm and 2mm
    ]
        \node[stage] (inputs) {Scalar\\modalities};
        \node[stage, right=of inputs] (hessians) {Multi-scale\\Hessians};
        \node[stage, right=of hessians] (fusion) {Normalize\\and fuse};
        \node[stage, right=of fusion] (graph) {Pairwise graph\\shape, curvature,\\alignment};
        \node[stage, below=2mm of graph] (pruning) {Threshold and\\components};
        \node[stage, left=of pruning] (mst) {Minimum\\spanning tree};
        \node[stage, left=of mst] (centrality) {Weighted\\centrality};
        \node[stage, left=of centrality] (skeleton) {Crack\\skeleton};

        \draw[flow] (inputs) -- (hessians);
        \draw[flow] (hessians) -- (fusion);
        \draw[flow] (fusion) -- (graph);
        \draw[flow] (graph) -- (pruning);
        \draw[flow] (pruning) -- (mst);
        \draw[flow] (mst) -- (centrality);
        \draw[flow] (centrality) -- (skeleton);
    \end{tikzpicture}
    \caption{Method overview. Local Hessian cues are fused across modalities and lifted to pairwise graph similarities; graph reduction and weighted centrality return the crack skeleton.}
...
\emph{Elongation.} Both endpoints must have a small \Frangi{} ratio:
\begin{equation}
S_{\mathrm{shape}}^\sigma(i,j)=
\exp\left[-\frac12\left(\frac{R_B(x_i)+R_B(x_j)}{s_{\mathrm{s}}}\right)^2\right].
\end{equation}

\emph{Positive curvature.} For dark ridges, the useful transverse curvature is $C_\sigma(x)=\lambda_2^\sigma(x)\mathbf 1_{\lambda_2^\sigma(x)>0}$. The intensity term is
\begin{equation}
...
\emph{Orientation alignment.} If $\theta^\sigma(x)$ is the angle of the longitudinal eigenvector, direction compatibility is modeled by
\begin{equation}
...
\begin{equation}
\overline S_{ij}=\max_{\sigma\in\Sigma}
S_{\mathrm{shape}}^\sigma(i,j)
S_{\mathrm{int}}^\sigma(i,j)
S_{\mathrm{align}}^\sigma(i,j),
\end{equation}
...
\begin{equation}
    S_{ij}=\max\{0,1-(1-\overline S_{ij})\rho_{ij}\},
    \qquad
    d_{ij}=1-S_{ij}.
\label{eq:distance}
\end{equation}
Thus the graph edge is strong only when both pixels are elongated, have the correct curvature sign and share a compatible orientation. The factor $\rho_{ij}$ in \eqref{eq:distance} regularizes the later tree extraction.

For FIND and the Palais des Papes, we use $s_{\mathrm{s}}=2$, $s_{\mathrm{i}}=1/4$, $s_{\mathrm{a}}=1/8$, $\Sigma=\{1,3,5,7\}$ and $R=12$. For the Vaches Noires images, the single-modal implementation uses $s_{\mathrm{s}}=1/2$, $s_{\mathrm{i}}=1/4$, $s_{\mathrm{a}}=1/8$, $\Sigma=\{1,3,5,7,9\}$ to cover wider visible fractures, while $R=3$ follows the preliminary study.

\subsection{Graph Extraction}
The extraction pipeline has four steps. First, thresholding removes low-confidence elements: Pixels are filtered by curvature, then low-similarity graph elements are pruned. The pruning rules used in the experiments are detailed below.
...
Third, a minimum spanning tree $\mathcal T$ \cite{Prim1957} is computed using the dissimilarity $d_{ij}$. The tree preserves the ``cheapest'' coherent connections and turns the locally thick response into a structure suitable for centerline extraction. 

Fourth, we compute a similarity-weighted tree centrality, related to betweenness centrality \cite{Freeman1977}. Unlike general-graph betweenness \cite{Brandes2001}, this score is computed in $O(|V|)$ with a breadth-first traversal. Each vertex receives the weight
\begin{equation}
...
The summary in Table~\ref{tab:components} makes explicit which failure mode each term is intended to control.

\begin{table}[!ht]
    \centering
    \caption{Role of the components in the extraction chain.}
    \label{tab:components}
    \begin{tabular}{p{0.27\linewidth}p{0.63\linewidth}}
        \toprule
        \textbf{Component} & \textbf{Role in the extraction chain} \\
        \midrule
        $S_{\mathrm{shape}}$ & Penalizes blob-like responses and favors elongated local structures. \\
        $S_{\mathrm{int}}$ & Strong positive curvature; closest part to classical \Frangi{} evidence. \\
        $S_{\mathrm{align}}$ & Penalizes locally incompatible directions and accidental bridges. \\
        $\rho_{ij}$ & Discourages long links between distant but visually similar pixels. \\
...
The scale set, radius and pruning cutoff are fixed per dataset. We set $\Sigma$ from the expected crack-width range and $R$ from the largest gap that should be bridged; the cutoff controls the sparsity--recall trade-off. For FIND and the Palais des Papes, the reproduction notebook applies the $0.60$ quantile separately to curvature magnitude and node-wise maximum similarity, retaining the upper $40\%$ at each step. For Vaches Noires, the hierarchical-tree filtration level is fixed \emph{ad hoc} at $\tau=0.30$, following the preliminary study \cite{BibiStrasbourg}.
...
\begin{figure}[!ht]
    ...
    \caption{FIND example. \textbf{Top:} Intensity, Range, \Frangi{} ratio and orientation. \textbf{Middle:} Positive curvature, similarity, largest component and weighted centrality. \textbf{Bottom:} Ground truth, CrackSegDiff, ours (63\% \textsc{Jaccard}, 71\% \textsc{Tversky}, 8\,px \textsc{Wasserstein}) and overlay.}
...
Figure~\ref{fig:RobustesseNoisy} shows an example at a strong noise level; Figure~\ref{fig:Results_noisy} reports the degradation curves. CrackSegDiff initially performs better, but its score decreases rapidly as noise increases. The proposed method starts lower but degrades more smoothly because the graph uses multi-scale curvature and orientation compatibility instead of learned appearance statistics alone.
...
\begin{figure*}[!t]
    ...
    \begin{subfigure}[b]{0.315\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0pt] (img) at (0,0) {\includegraphics[width=0.84\linewidth]{Jaccard_noisy.png}};
    ...
    \begin{subfigure}[b]{0.315\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0pt] (img) at (0,0) {\includegraphics[width=0.84\linewidth]{Tversky_noisy.png}};
            \useasboundingbox (img.south west) rectangle (img.north east);
            \draw[-latex, thin] (img.south west) -- (img.south east) -- ++(12pt,0) node[above left, inner sep=1pt, font=\scriptsize] {Noise};
            \draw[-latex, thin] (img.south west) -- (img.north west) -- ++(0,12pt) node[above, font=\scriptsize] {Score (\%)};
            \node[below left, font=\scriptsize] at (img.south west) {$0$};
    ...
    \begin{subfigure}[b]{0.315\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0pt] (img) at (0,0) {\includegraphics[width=0.84\linewidth]{Wasserstein_noisy.png}};
            \useasboundingbox (img.south west) rectangle (img.north east);
            \draw[-latex, thin] (img.south west) -- (img.south east) -- ++(12pt,0) node[above left, inner sep=1pt, font=\scriptsize] {Noise};
...
\begin{figure}[!tb]
    \centering
    \includegraphics[width=0.80\linewidth]{Test1_512_512_result.png}
    \caption{Vaches Noires \texttt{Test1}. Ground truth: green; ours: red; U-Net + transfer learning: blue.}
    \label{fig:vaches_resultats1}
\end{figure}

\begin{figure}[!tb]
    \centering
    \includegraphics[width=0.80\linewidth]{Test2_512_512_result.png}
    \caption{Vaches Noires \texttt{Test2}. Ground truth: green; ours: red; U-Net + transfer learning: blue.}
...
The code and reproduction notebook are available in the \href{https://github.com/Ludwig-H/Generalized-Frangi-for-Automatic-Crack-Extraction-on-FIND-dataset}{project repository}.
...
The output is an editable spatial graph, which is useful for semi-automatic annotation. Vision foundation models are now essential baselines for crack segmentation; a direct comparison with SAM \cite{Kirillov2023SAM} and its crack-specific adaptation CrackSAM \cite{Ge2024CrackSAM} is therefore a priority. We will also test sensitivity on established datasets \cite{shi2016automatic} and higher-order graph variants \cite{BibiAppliedNetworkScience}.

\section*{Acknowledgment}
The work of the Inria authors was partially supported by Bpifrance through the LiChIE contract (2020--2025). L. Hauseux was supported by Université Côte d'Azur through DS4H (ANR-17-EURE-0004) and 3IA (ANR-19-P3IA-0002).
