...
The extraction pipeline has four steps. First, thresholding applies one retention factor $\tau$ to both graph elements: It keeps the strongest fraction $\tau$ of edges by $S_{ij}$, ranks nodes by maximum incident similarity, and keeps their strongest fraction $\tau$.
...
The editable spatial-graph output supports semi-automatic annotation. Vision foundation models are now essential baselines for crack segmentation; a direct comparison with SAM \cite{Kirillov2023SAM} and its crack-specific adaptation CrackSAM \cite{Ge2024CrackSAM} will therefore be our priority. We will also test sensitivity on established datasets \cite{shi2016automatic} and higher-order graph variants \cite{BibiAppliedNetworkScience}.
