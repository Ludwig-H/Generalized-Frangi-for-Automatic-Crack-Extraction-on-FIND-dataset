...
\begin{itemize}\setlength\itemsep{2.6mm}
 ...
 \item \textbf{Polarity}: $F_\sigma=0$ when $\lambda_2\le 0$
\end{itemize}
\vspace{1mm}
\[
...
\vspace{-2mm}
\[
 d_{ij}=\min\{1,\rho_{ij}(1-S^{(0)}_{ij})\},\qquad S_{ij}=1-d_{ij},\qquad
 0<\rho_{ij}=\|x_i-x_j\|_2\le R
\]
\reffoot{euvip26}
\end{frame}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.1cm}ccc@{}}\toprule
...
\panel{.38}{IntensityNoisy_1FIND.png}{Noisy intensity}\hfill
\panel{.38}{ResultatNoisy_1FIND.png}{Our result, $s=0.5$}
\par\vspace{.5mm}
\begin{itemize}
