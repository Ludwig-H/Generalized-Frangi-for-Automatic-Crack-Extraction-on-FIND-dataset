...
\author{
    ...
    \IEEEauthorblockA{
        ...
        °\textit{Cerema}, {\nom{Endsum} research team}, France\\
...
In this paper, we propose a ``universal'', training-free approach that robustly extracts cracks across various data modalities (\eg intensity and range). Our method generalizes the classical \Frangi{} vesselness filter. 
Instead of a pixel-wise response, we construct a sparse graph driven by a pairwise \Frangi{} similarity metric, which rigorously encodes local tubular geometry (elongation, contrast, and orientation alignment). 
...
to provide the precise topological skeleton of the main crack network.

Evaluated on the FIND benchmark and illustrated on geological data (both real-world datasets), our unsupervised method proves to be efficient ($\sim$10s per image on Colab CPU) and demonstrates superior robustness to synthetic noise and domain shifts compared to supervised diffusion models. 
...
Historically tackled via Hessian-based filtering \cite{Frangi1998, Sato1998}, crack detection transitioned to deep learning (DL) over a decade ago. From CNNs \cite{Unet2015, DeepCrack2019} to transformers \cite{CrackFormer2021} and state-of-the-art diffusion models \cite{CrackSegDiff2024} (achieving impressive F1-scores $>90\%$), DL methods essentially treat the problem as a supervised task on closed domains. Consequently, they remain computationally expensive, data-hungry, and suffer from a critical lack of robustness when confronted with unseen modalities or noise patterns. 
...
A recent comprehensive review by Zhang \textit{et al.}  highlights %a growing crisis in the field: 
the ``data efficiency'' and ``generalizability'' gaps \cite{Zhang2025Review}. Supervised models are often fragile ``black boxes'' that struggle with domain shifts when the training and test data distributions differ (\textit{e.g.} changes in lighting, sensor noise, or surface material). Collecting and annotating expert-level datasets for every new scenario is prohibitively expensive and there exists no large-scale dataset yet, except maybe the forthcoming \emph{3DCrack} \cite{3DCrack2026, Zhang2025Review} database. So, there remains an urgent need for ``universal'' methods %that enable zero-shot transfer: 
that work robustly on unseen data with no training, and minimal priors. 


Building on our preliminary work in \cite{BibiStrasbourg} and addressing % these limitations, 
the limitations of DL methods, we propose to revisit and significantly extend model-based approaches. We introduce a fully unsupervised pipeline that fuses multi-modal data into a unified geometric graph representation. 
We hypothesize that although noise patterns differ across modalities (\eg speckle in intensity \textit{vs}. Gaussian in range), the physical geometry of the crack produces signal variations (eigenvectors) that align constructively. 
...
Early crack detection relied on heuristic thresholding and edge detection (\textsc{Sobel}, \textsc{Canny}). To address the specific geometry of elongated structures similar to cracks, ``vesselness'' filters were developed in the medical imaging community. The seminal work of \Frangi{} \textit{et al.} \cite{Frangi1998} uses the eigenvalues of the Hessian matrix to estimate the probability of a pixel belonging to a tubular structure. However, these methods operate locally (pixel-wise) and are prone to fragmentation in noisy environments. Our approach bridges this gap by embedding the \Frangi{} response into a graph structure, considering pairs of pixels to enforce continuity and geometric information.

\subsection{The Deep Learning Era}
Architectures like U-Net \cite{Unet2015} and DeepCrack \cite{DeepCrack2019} set the initial benchmarks for DL methods, with a certain success. Recent advances focus on generative models. CrackSegDiff \cite{CrackSegDiff2024} uses a diffusion process to generate crack masks, achieving high performance on specific datasets. However, as noted by Zhang \textit{et al.} \cite{Zhang2025Review}, these models often fail when the data change (\eg applying a model trained on optical images to SAR or noisy depth data), highlighting the need for methods that depend less on massive, specific training datasets.
...
Modalities are seen as signals from the domain (\ie pixel grid) $\Omega \subset\mathbb R^2$ to $\mathbb R$.
For each modality $m$ and \Frangi{} scales $\sigma \in \Sigma$, the Hessian matrix $\Hess_{\sigma}^{(m)}(\mathbf{x})$ is computed \emph{via} convolution with a Gaussian kernel of standard deviation $\sigma$. To handle disparate dynamic ranges, we normalize by the maximum spectral norm:
\begin{equation}
    \hat{\Hess}_{\sigma}^{(m)}(\mathbf{x}) = \frac{\Hess_{\sigma}^{(m)}(\mathbf{x})}{\max_{\mathbf{y} \in \Omega} \matnorm{\Hess_{\sigma}^{(m)}(\mathbf{y})} }.
\end{equation}

The \textit{Fused Hessian} is a % weighted sum ou linear combination
linear combination:
\begin{equation}
...
We define a pairwise similarity $S_{ij} \in [0, 1]$ combining three terms defined at scale $\sigma \in\Sigma$ by:
...
The final similarity is $S_{ij} = \max_{\sigma \in\Sigma} \bigl(\mathcal{S}_{\text{shape}}^\sigma \cdot \mathcal{S}_{\text{int}}^\sigma \cdot \mathcal{S}_{\text{align}}^\sigma\bigr)$. We embed this into a dissimilarity space using the transformation:
\begin{equation}
...
As illustrated in Fig.~\ref{fig:similarity}, the proposed % graph : graph
similarity significantly cleans the response compared to the standard pixel-wise \Frangi{} filter (approx. Fig.~\ref{fig:frangi}). This improvement is largely driven by the alignment term (Fig.~\ref{fig:angle}), which creates a sharp cutoff at fault boundaries, effectively isolating the backbone.

\subsubsection{Parameter values}

We fixed 
$ s_{\mathrm{s}} \gets 2, s_{\mathrm{i}} \gets \tfrac14, s_{\mathrm{a}} \gets \tfrac18 $ (these parameters can be interpreted as standard deviations of Gaussian kernels) and the set of \Frangi{} scales $\sigma$ is taken as $ \Sigma \gets \{1,3,5,7 \}$.

These parameters were determined empirically and are quite close to the ones chosen in \cite{BibiStrasbourg}. A larger value was selected for $s_{\mathrm s}$ to account for the irregularity of the \Frangi{} ratio (see Fig.~\ref{fig:frangiRatio}). The results are relatively insensitive to small variations in these parameters.
...
We compute the minimum spanning tree on the largest component (assuming the crack network to be unique). This topological reduction transforms the data into a very soft tree structure. The minimum tree preserves the most significant connections (lowest $d_{ij}$) while acting as a skeletonization pre-process, enabling extremely fast computation of centrality (next step).


\subsubsection{Step 3, Weighted Betweenness Centrality}
To extract the pixel-precise backbone from the minimum tree, we compute the {\emph{weighted} betweenness centrality} %\cite{Freeman1977, 
\cite{Brandes2001}. 
In the specific case where the graph is a tree $\mathcal T$, the centrality $C_B(v)$ of a node $v$ takes a very simple form. 

Let $\text{Branch}(\mathcal{T}\setminus v)=\{b_1,\dots,b_k\}$ denote the set of connected components (branches) obtained by removing $v$ from $\mathcal{T}$. 
Then the betweenness centrality of a node $v \in \mathcal T$ is
\begin{equation}
    C_B(v) \,=\, \sum_{1\le r<s\le k} \bigl|\mathrm{Nodes}(b_r)\bigr|\,\bigl|\mathrm{Nodes}(b_s)\bigr|.
\end{equation}

For each branch $b\in \mathrm{Branch}(\mathcal{T}\setminus v)$ (with $v$ included back as the root), the unweighted branch size can be written as a counting sum over its edges, $\sum_{\{i,j\}\in \mathrm{Edges}(b)} 1$, which we replace by the \emph{similarity}-weighted sum $$\sum_{\{i,j\}\in \mathrm{Edges}(b)} S_{ij}.$$

Nodes located on the geometric ``spine'' of the tubular structure minimize the travel cost and act as bridges for the vast majority of paths. These nodes accumulate very high centrality scores. Conversely, nodes on the periphery (the ``flesh'' of the crack width) lie on few (and low-weighted) shortest paths and have low $C_B$.
See Fig.~\ref{fig:betweenness}.

\subsubsection{Step 4, Adaptive Pruning}
Finally, we differentiate the backbone from minor branches. The tree $\mathcal{T}$ is rooted at the node maximizing the centrality score. We then recursively explore the tree starting from this root; branches with insufficient centrality are pruned. This process, which involves two threshold parameters, collapses the ribbon structure into a precise, 1-pixel wide skeleton.
...
We evaluate the robustness of our algorithm: quantitatively using synthetic noise and qualitatively via domain transfer. We use the FIND dataset \cite{FIND2022}, which is, to the best of our knowledge, the only public benchmark providing aligned Intensity and Range modalities suitable for this task. 
...
We excluded 23 images whose ground truth annotations were inconsistent with the physical fracture network (\eg multiple thin faults merged in the ground truth, artifacts or inverted contrast), ensuring a fair evaluation of the geometric algorithm.
...
\begin{figure}[!t]
    ...
    \caption{Processing of image 1 in the FIND dataset. 
    \textbf{Top row:} (a) Intensity and (b) Range modalities, (c) \Frangi{} ratio $\mathcal{R}_B = |\lambda_1| / |\lambda_2|$ and (d) map of $\sin(\theta)$ associated to the Hessian. 
    \textbf{Middle row:} (e) Map $ \mathbf x \mapsto \lambda_2^{} \mathbf 1_{ \lambda_2^{} > 0} $ (very close to the original \Frangi{} filter), (f) Our \Frangi{} similarity ($\mathbf x_i \mapsto \max_{j, \mathbf x_i \sim \mathbf x_j} S_{ij}$), (g) the largest cluster and (h) the weighted betweenness centrality. 
    \textbf{Bottom row:} (i) Ground truth, (j) Result of CrackSegDiff on fused modalities, (k) Our resulting skeleton (\textsc{Jaccard} / IoU: 63\%; \textsc{Tversky}: 71\%; \textsc{Wasserstein}: 8\,px) and (l) Overlay of ground truth, CrackSegDiff and our skeleton.}
...
We evaluate robustness by injecting synthetic noise consistent with sensor physics (see Fig.~\ref{fig:intensityNoisy}~and~ \ref{fig:rangeNoisy}): multiplicative speckle noise for Intensity (simulating coherent imaging artifacts); additive white Gaussian noise for Range (simulating thermal noise).
%\begin{itemize}
...
\begin{figure}[!th]
    ...
    \caption{Noisy settings. From left to right: (a) Noisy Intensity (var $=0.5$) and (b) Noisy Range (sigma $= 0.5$) modalities of image $1$ in FIND dataset, (c) the \Frangi{} similarity and (d) the result of our algorithm on these noisy modalities.}
...
To ensure a fair comparison, we apply morphological \textsc{Lee} skeletonization \cite{SciKitSkeletonization} to both the ground truth masks and the CrackSegDiff predictions. (See our \href{https://colab.research.google.com/github/Ayana-Inria/Generalized-Frangi-for-Automatic-Crack-Extraction-on-FIND-dataset/blob/main/FIND_Frangi_Fusion_Avignon_Colab.ipynb}{notebook} for technical details.)

% De quel Notebook parle-t-on ? 
...
We compare our method against CrackSegDiff \cite{CrackSegDiff2024}. %As shown in Table~\ref{tab:ResultatsClean}, CrackSegDiff outperforms our method on clean data. 
CrackSegDiff outperforms our method on clean data. This is expected, as the diffusion model learns to fit the training distribution perfectly (Tab.~\ref{tab:ResultatsClean}, see also Fig.~\ref{fig:GT}~and~\ref{fig:CSD}).  
...
The FIND dataset includes a ``Filtered'' modality (pre-processed Range, see~\cite{FIND2022}). Integrating this modality increases the performance of our algorithm (see Tab.~\ref{tab:ResultatsAblation}).
With a \textsc{Tversky} index of 71\% and a Wasserstein distance of 11\,px, our unsupervised method effectively isolates the crack topology. These metrics demonstrate a high-fidelity recovery of the underlying structure (see Fig.~\ref{fig:Resultat}) without requiring any training.

\begin{table}[!h]
    \centering
    \caption{Performance of our algorithm on varying the modality weights. $I$: Intensity, $R$: Range, $F$: Filtered.}
...
%\textcolor{red}{Fusionner : clean data = CSD outperforms. }
...
\begin{figure}[!h]
    ...
    \caption{The results on \emph{noisy} (both Intensity and Range) FIND (average scores and standard deviation). From top to bottom: (a) \textsc{Jaccard} index, (b) \textsc{Tversky} index and (c) \textsc{Wasserstein} distance. In red, \textcolor{myred}{our algorithm}; in green \textcolor{mygreen}{CrackSegDiff} results. $x$-axis: the noise, from $0$ to $0.5$ (see \href{https://colab.research.google.com/github/Ayana-Inria/Generalized-Frangi-for-Automatic-Crack-Extraction-on-FIND-dataset/blob/main/FIND_Frangi_Fusion_Avignon_Colab.ipynb}{notebook} for technical details). $y$-axis: performance, from 0\% to 100\% (by step 10\%) for (a) and (b), from 0\,px to 50\,px (by step of 5\,px) for (c).}
    \label{Results_noisy}
\end{figure}
...
%To illustrate the zero-shot capabilities of our algorithm, we present results on a fractured limestone wall forming part of the support structure of the \textit{Palais des Papes} (Avignon, France). Eight images were captured with a smartphone. % (iPhone SE, 24 MP, 5712 × 4284 pixels). 
%A point cloud was reconstructed via photogrammetry using the Metashape commercial software, from which both a digital depth model and an orthoimage were derived (Fig.~\ref{PalaisDesPapes}). 

To illustrate the adaptation capabilities of our algorithm, we present results on a dataset completely different from the previous one. We consider a fractured limestone wall forming part of the support structure of the \textit{Palais des Papes} (Avignon, France). From eight images, captured with a smartphone, a digital depth model and an orthoimage were derived using the Metashape commercial software (Fig.~\ref{PalaisDesPapes}). 

The wall features a primary open fracture several centimeters deep, extending vertically from top to bottom. Several shallower, less open fractures intersect it almost perpendicularly. This wall poses significant challenges for image processing due to its very light surface, the diversity of fracture shapes and orientations, and subtle variations in depth and texture.
...
While minor fractures are missed, due to the resolution limitations of the depth sensor, the algorithm successfully extracts the main crack network without requiring any specific adjustments. Similar robustness was demonstrated on geological data from the \emph{Vaches Noires} cliffs \cite{BibiStrasbourg}.

%[Mentionner le Gretsi et les Vaches noires \cite{BibiStrasbourg}.]

\section{Conclusion}
We presented a versatile, unsupervised framework for crack extraction: by generalizing the \Frangi{} filter to multi-modal graphs and leveraging topological centrality, we obtain a robust training-free extraction method, with minimal parameter tuning. Our geometric approach offers superior stability under severe noise, is readily adaptable to different numbers of modalities and successfully handles domain shifts. It generates editable geometric proposals for fine skeleton-based crack delineation, and may also serve as an annotation tool for building large-scale datasets for future learning-based research. 

% A garder ??? C'est finalement assez peu informatif
% While instantiated with \Frangi, our framework establishes a generic paradigm -- lifting local responses into informational graphs to recover structured objects -- a methodology that might be extended to other problems in future work. 



%By generalizing the \Frangi{} filter to multi-modal graphs and leveraging topological centrality, we achieve robust extraction without training, cho only few parameters. %The Avignon case study confirms the method's versatility. 
%(\eg \Frangi{} filter, \textsc{Steger} filter for centerlines, superpixel affinities for regions).
%This yields a versatile 'human-in-the-loop' tool, generating \emph{editable} geometric proposals for \emph{annotation tasks}, from precision skeletonization to image segmentation.

% Future work will apply this pipeline to the massive 3DCrack dataset \cite{Zhang2025Review, 3DCrack2026}. 
%We also emphasize that while our method is robust, high-quality pre-processing (like the FIND Filtered modality) remains a critical factor for maximizing topological fidelity.
...
\begin{itemize}
    \item Our \Frangi{}-Generalized \href{https://github.com/Ayana-Inria/Generalized-Frangi-for-Automatic-Crack-Extraction-on-FIND-dataset}{GitHub repo} \cite{GitHubGeneralizedFrangi2026} containing the code and \href{https://colab.research.google.com/github/Ayana-Inria/Generalized-Frangi-for-Automatic-Crack-Extraction-on-FIND-dataset/blob/main/FIND_Frangi_Fusion_Avignon_Colab.ipynb}{Colab python notebook} reproducing all tables and figures present in this paper. This notebook provides also the FIND \cite{FIND2022} benchmark and our \textit{Palais des Papes} test. The users need a Google account to reproduce on their Drive our architecture, providing the CrackSegDiff masks. These masks can be generated using our corrected CrackSegDiff GitHub fork (see next).
    \item Our corrected CrackSegDiff \href{https://github.com/Ayana-Inria/CrackSegDiff}{GitHub fork} with an additional Colab python notebook that automatically trains and generates the masks. 
\end{itemize}
}
