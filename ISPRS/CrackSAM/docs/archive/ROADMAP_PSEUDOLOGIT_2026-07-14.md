# Archive au 14 juillet 2026 — piste pseudo-logit abandonnée

> Ce plan est conservé comme provenance de l'expérience négative. La roadmap
> active est [`../04_IMPLEMENTATION_ROADMAP.md`](../04_IMPLEMENTATION_ROADMAP.md).

# Roadmap : Migration de CrackSAM vers SAM 2 avec guidage par Frangi-Graphe (GPU)

> [!IMPORTANT]
> **Consignes initiales pour ChatGPT** :
> 1. Avant de commencer à coder quoi que ce soit, lis attentivement le papier scientifique de référence fourni : [CrackSAM.pdf](../../reference/papers/CrackSAM.pdf). Ce papier détaille la méthodologie de fine-tuning PEFT (LoRA/Adapter) et sert de base scientifique à ce projet.
...
*   **Code originel CrackSAM (SAM 1 + Adapters/LoRA)** : [`../../reference/cracksam1/code/`](../../reference/cracksam1/code/)
    *   [train.py](../../reference/cracksam1/code/train.py) : Script d'entraînement principal.
    *   [test.py](../../reference/cracksam1/code/test.py) : Script d'évaluation.
    *   [trainer.py](../../reference/cracksam1/code/trainer.py) : Boucle d'entraînement.
    *   [datasets/dataset_khanhha.py](../../reference/cracksam1/code/datasets/dataset_khanhha.py) : Dataloader pour le jeu de données d'entraînement.
    *   [utils.py](../../reference/cracksam1/code/utils.py) : Fonctions de perte (Dice, Focal) et calcul des métriques de segmentation.
...
ChatGPT doit coder l'évaluation sur l'ensemble de test en calculant les métriques suivantes (définies dans [utils.py](../../reference/cracksam1/code/utils.py) et `test_k2_clean.py`) :
