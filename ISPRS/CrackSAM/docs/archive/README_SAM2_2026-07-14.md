# Archive au 14 juillet 2026 — ne pas utiliser comme guide actif

> Ce document conserve le protocole historique du pseudo-masque Frangi. Les
> chemins et la recommandation d'architecture ont été remplacés par le
> [README actuel](../../README.md) et la
> [feuille de route FrangiGraph-Residual](../04_IMPLEMENTATION_ROADMAP.md).
...
Le code SAM 1 d'origine est désormais archivé sous `reference/cracksam1/code/`.
...
nohup bash ISPRS/CrackSAM/workflows/run_full_cracksam2_experiment.sh \
...
python ISPRS/CrackSAM/precompute_frangi_prompts.py \
  ...
  --list-file ISPRS/CrackSAM/protocol/cracksam_paper/lists/lists_khanhha/train.txt \
...
```bash
bash ISPRS/CrackSAM/workflows/run_milestone_comparison.sh evaluate
python -m pip install -r ISPRS/CrackSAM/requirements-report.txt
PYTHONPATH=ISPRS/CrackSAM python ISPRS/CrackSAM/reporting/generate_frangi_milestone_report.py \
