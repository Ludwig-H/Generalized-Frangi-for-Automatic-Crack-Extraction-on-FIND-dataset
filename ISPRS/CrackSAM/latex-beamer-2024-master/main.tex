...
\begin{frame}{Inspiration : Consistance Géométrique (Luo et al., 2026)}
    \begin{block}{Principes et Constats}
        \begin{itemize}
            \item \textbf{Limites de l'apprentissage profond} : La recherche globale non contrainte (\emph{unconstrained matching}) est trompeuse à cause des structures répétitives et de l'auto-similarité.
            \item \textbf{La géométrie comme guide} : Restreindre la recherche à l'espace physiquement cohérent (bande épipolaire) élimine les faux positifs.
            \item \textbf{Découplage Ressemblance/Précision} : Les modèles 3D complexes améliorent l'accord sémantique global mais lissent les détails locaux, tandis que les modèles 2D robustes (\emph{SAM}, \emph{DINO}) conservent une meilleure précision géométrique locale.
        \end{itemize}
    \end{block}
