...
\begin{frame}{Proposition : CrackSAM Guidé par la Géométrie}
    \begin{block}{Repenser l'Entrée du Modèle (Fusion Précoce)}
        ...
        \begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
            % Nodes for channels
            \node[draw, fill=gray!15, minimum width=4.5cm, minimum height=0.5cm, rounded corners=2pt, align=center] (c1) at (0, 1.0) {\scriptsize \textbf{Canal 1 (Visuel)} : Optique en gris};
            \node[draw, fill=red!10, minimum width=4.5cm, minimum height=0.5cm, rounded corners=2pt, align=center] (c2) at (0, 0.3) {\scriptsize \textbf{Canal 2 (Physique)} : Thermique / Range};
            \node[draw, fill=blue!10, minimum width=4.5cm, minimum height=0.5cm, rounded corners=2pt, align=center] (c3) at (0, -0.4) {\scriptsize \textbf{Canal 3 (Géométrique)} : Distance du graphe};

            % Concat node
            \node[draw, fill=orange!15, minimum width=2.4cm, minimum height=2.1cm, rounded corners=4pt, align=center] (concat) at (4.6, 0.3) {
            ...
            % Arrow to SAM
            \draw[-stealth, ultra thick, myred] (concat.east) -- (7.2, 0.3) node[above, midway] {\scriptsize Entrée};

            % SAM block
            \node[draw, fill=mygreen!20, minimum width=3.0cm, minimum height=1.6cm, rounded corners=4pt, thick, align=center] (sam) at (9.0, 0.3) {
                \textbf{CrackSAM}\\
    ...
    \begin{block}{Avantages Attendus}
        \begin{itemize}
            \item \textbf{Zéro modification structurelle} : Le modèle \emph{SAM} d'origine accepte nativement l'image ainsi formatée.
            \item \textbf{Apprentissage LoRA synergique} : Les adaptateurs apprennent à associer le prior géométrique (Canal 3) à la présence physique de fissures, tout en exploitant les canaux bruts (1 et 2) pour détourer précisément les contours.
        \end{itemize}
    \end{block}
\end{frame}
