...
NAVY, RED, GRAY = "#243A59", "#D7263D", "#627186"
PALE, PINK, LINE = "#F3F6FA", "#FFF5F6", "#CDD5E0"
...
def box(x, y, w, h, s="", fill="white", edge=LINE, size=15, bold=False):
...
# Editorial heading matches the navy/red palette of the thesis presentation.
ax.plot([.55, .55, .88], [8.06, 8.58, 8.58], color=RED, lw=4)
text(1.03, 8.42, "SAM fournit les représentations ;", 26, weight="bold", ha="left")
text(1.03, 7.88, "Frangi organise leurs relations", 26, weight="bold", ha="left")
...
arrow((5.53, 4.51), (8.72, 4.51), color=RED, lw=2)
text(7.14, 5.06, "Niveau du premier\ngroupe commun", 12, RED,
     bbox={"facecolor": "white", "edgecolor": "none", "pad": 2})
text(10.66, 3.58, "score(i, j) = score SAM(i, j) + biais du groupe commun",
     14, color=NAVY)
box(6.36, 2.38, 8.55, .78,
    "3 coefficients appris, initialisés à zéro\nFusion fine / intermédiaire / grossière · partagés entre les têtes",
    fill=PALE, edge=LINE, size=13)

text(.61, 1.51, "Le biais est partagé ; les coefficients d’attention restent distincts.",
     17, weight="bold", ha="left")
text(.61, 1.08, "Deux positions proches dans l’arbre peuvent ainsi échanger davantage.",
     15, ha="left")
ax.plot([.6, 15.4], [.66, .66], color=LINE, lw=1)
text(.61, .35, "Perspective — aucun gain mesuré", 13, RED, weight="bold", ha="left")
...
for suffix in ("svg", "png"):
    ...
    fig.savefig(destination, facecolor="white", dpi=180, metadata=metadata)
    if suffix == "svg":
